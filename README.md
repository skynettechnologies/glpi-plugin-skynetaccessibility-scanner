...
* **GLPI 10.0+** (or later)
* **PHP 8.1+** (PHP 8.2 or higher recommended)
* **MySQL 5.7+** or **MariaDB 10.3+**
* **Web Server:** **Apache 2.4+** or **Nginx** (with `mod_rewrite` enabled for Apache)
...
* **Access permissions** to the `plugins/` directory within the GLPI installation
* **Command-line access (CLI)** to execute installation or update scripts if needed
...
1. **Download & Extract**
   ...
   * Download **`skynetaccessibilityscanner.zip`** from the official source.
   * Place it in the GLPI `plugins/` directory (root of platform) and unzip:

     ```
     /plugins/skynetaccessibilityscanner/
     ```

2. **Set Permissions**

   ```bash
   sudo chown -R www-data:www-data /plugins/skynetaccessibilityscanner
   sudo chmod -R 755 /plugins/skynetaccessibilityscanner
   ```

3. **Enable & Manage Plugin**

   * Log in to GLPI as **super-admin**.
   * Go to **Setup → Plugins**.
   * Find **SkynetAccessibility Scanning**, click **Install**, then **Enable**.
   * To manage the plugin settings, click the **Configure** or **Manage** button next to it in the Plugins list.

4. **Verify**

   * The plugin should now appear in GLPI’s menu.
   * Open the plugin via its menu or the **Manage** button to start using it.
